...
int main ( int argc, char *argv[] ) {
  ...
  for (int k = 0; k < n; k++){
    ...
    if (row_rank == root){
      if((row_rank + 1) < row_size){
        MPI_Send(sendColBuff.data(), sendColBuff.size(), MPI_INT, row_rank+1, 0, row_comm);
      }
      if ((row_rank - 1) >= 0){
        MPI_Send(sendColBuff.data(), sendColBuff.size(), MPI_INT, row_rank-1, 0,row_comm);
      }
    ...
    if (col_rank == root){
      if((col_rank + 1) < col_size){
        MPI_Send(sendRowBuff.data(), sendRowBuff.size(), MPI_INT, col_rank+1, 0, col_comm);
      }
      if((col_rank - 1) >= 0){
        MPI_Send(sendRowBuff.data(), sendRowBuff.size(), MPI_INT, col_rank-1, 0, col_comm);
      }
    ...
    if (row_rank != root){
      MPI_Recv(sendColBuff.data(), n/sqrtp, MPI_INT, MPI_ANY_SOURCE, 0, row_comm, &status);
      int sender = status.MPI_SOURCE;
      if (sender > row_rank && (row_rank - 1) >= 0){
        MPI_Send(sendColBuff.data(), sendColBuff.size(), MPI_INT, row_rank-1, 0,row_comm);
      }
      else if (sender < row_rank && (row_rank + 1) < row_size)
      {
        MPI_Send(sendColBuff.data(), sendColBuff.size(), MPI_INT, row_rank+1, 0,row_comm);
      }
    ...
    if (col_rank != root){
      MPI_Recv(sendRowBuff.data(), n/sqrtp, MPI_INT, MPI_ANY_SOURCE, 0, col_comm, &status);
      int sender = status.MPI_SOURCE;
      if (sender > col_rank && (col_rank - 1) >= 0){
        MPI_Send(sendRowBuff.data(), sendRowBuff.size(), MPI_INT, col_rank-1, 0, col_comm);
      }
      else if (sender < col_rank && (col_rank + 1) < col_size)
      {
        MPI_Send(sendRowBuff.data(), sendRowBuff.size(), MPI_INT, col_rank+1, 0, col_comm);
      }
    }
